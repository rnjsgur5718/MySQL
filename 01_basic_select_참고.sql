...
- 같은 컬럼을 여러번 조회할 수 있다.
************************************** */
select 10 as a, 20 as b; -- select 절에 상수값을 넣을 넣어서 확인. 이때 from 생략 
select '안녕하세요' as 인사말;
...
/* *************************************
where 절을 이용한 행 선택 
where 컬럼  연산자  피연산자값   - 컬럼의 값들과 연산했을 때 True 인 행들만 조회 =>> Filter 개념.
************************************* */
...
select  emp_name
from 	emp
where   emp_name like '___e%';  -- _: 1글자 
...
-- where not (dept_name in ('Shipping', 'Finance'));
...
-- () 를 묶지 않으면 and 가 우선이 되어서 (job like '%MAN% and salary <= 1000) 조건을 만족하거나 (year(hire_date) >= 2008)  조건을 만족하는 행을 선택하게 됨.
...
select  emp_id,
        ...
        salary
from    emp 
order by  job, salary desc, emp_name desc; 
-- 1차로 job 오름차순 정렬, 2차로 salary 내림차순 (job이 같은 것 안에서 정렬), 3차로 emp_name 내림차순(job, salary 둘다 같은경우)

-- 부서명(dept_name)의 오름차순으로 정렬해 조회하시오.
select  *
from    emp
...
from    emp
where   salary > 5000
order by 3 desc; --  select 절의 3번째 컬럼을 기준으로 정렬해라.
-- order by  salary desc;
