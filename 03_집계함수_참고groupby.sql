...
select  count(*), count(emp_id) from emp;  # emp 테이블의 행 수
select count(salary) "salary 컬럼의 데이터수",
       count(comm_pct) "comm_pct 컬럼의 데이터수" -- null 을 뺀 원소의 개수. 
 from emp;

-- comm_pct 평균
select avg(comm_pct), sum(comm_pct)/35, sum(comm_pct)/107, avg(ifnull(comm_pct, 0))
from emp;

select ifnull(comm_pct, 0) from emp;

-- EMP 테이블에서 가장 최근 입사일(hire_date)과 가장 오래된 입사일을 조회
select  max(hire_date) "최근입사일", 
		min(hire_date) "최초입사일"
from    emp;        
...
-- TODO: 커미션 비율(comm_pct)이 없는 직원의 수를 조회 -- null값의 개수를 세는 함수는 없다. 
...
select round(avg(comm_pct), 2), truncate(avg(comm_pct), 2)
from   emp;
...
select count(distinct dept_name) from emp; -- null을 빼고 count


select count(distinct ifnull(dept_name, 0)) from emp;

/*
null: 모르는 값 (수집이 안된 값)
null: 어떤 의미를 가지는 값으로 사용. => 없다, 0 (이경우에는 count할때 개수에 포함되야 한다.)
*/
...
-- 입사연도 별 직원들의 급여 평균.
select  year(hire_date), round(avg(salary), 2) "급여평균"
from    emp
...
select char_length(emp_name), sum(salary)
from   emp
group by char_length(emp_name);
