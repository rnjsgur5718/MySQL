...
select char_length('aaaaaa'),  char_length('가나다라ㄱㅏ'); -- 글자수
...
select insert('123456789', 2, 5, '안녕하세요'); -- 2번째 글자부터 5글자를 '안녕하세요' 변경
select replace('aaaaabbbbbaaaabbbbbcccccddddaaaa', 'a', '가'); -- a 를 모두 '가' 로 변경. 여러글자 지정할 수 있다.
...
select substring('1234567890', 4, 3); -- 4번째 글짜부터 3글자만 반환
select substring('1234567890', 4); -- 4번째 글자부터 나머지 모두 반환
select trim('       aaaa      ') as "v"; --  좌우공백 제거
select ltrim('       aaaa      ') as "v"; -- 왼쪽공백제거
select rtrim('       aaaa      ') as "v"; -- 오른쪽 공백제거
...
select emp_name, char_length(emp_name) "이름 글자수"
from   emp
...
select  round(50.12), -- 정수. 위치자리 0
		round(50.56),
		round(50.56789, 2), -- 소수점 2번째 자리 아래서 반올림
        round(50.56789, 0),
        round(567890.56789, -2);
;
select  truncate(50.12, 0), -- 내림
		truncate(50.67, 0),
        truncate(50.12233445, 3),
        truncate(567889, -3)
        ; -- 정수 
...
select hour(now()), minute(now()), second(now()), microsecond(now()); -- 1/1000 초
...
select  adddate(hire_date, interval -10 day),
...
select  adddate(hire_date, interval -6 month), 
		hire_date, 
...
select timestampdiff(day, '2011-03-20', '2022-05-26');  #뒤 - 앞
...
select  dayofweek(hire_date) "입사요일",
		concat(substring('월화수목금토일', dayofweek(hire_date), 1), '요일')   "입사요일2"
from    emp
where   emp_id = 100;
-- where emp_id in (100, 110,120, 200);
